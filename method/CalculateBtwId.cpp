...
void* get_feature_byid(int id, OGRLayer* currentlyr, const char* type, const char* fieldname)
{
	...
	while ((poFeature = currentlyr->GetNextFeature()) != NULL)
	{
		if (poFeature->GetFID() == id) // find the feature
		{
			...
			{
				OGRPoint pt;
				poFeature->GetGeometryRef()->Centroid(&pt);
				pt.assignSpatialReference(poFeature->GetGeometryRef()->getSpatialReference()->Clone());
...
double get_poly_distance(int id1, int id2, OGRLayer* currentlyr)
{
	...
	p[1] = *(OGRPoint*)get_feature_byid(id2, currentlyr, "centroid");
	double dis;
